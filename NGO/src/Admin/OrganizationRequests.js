...
const OrganizationRequests = () => {
  ...
  const handleAcceptOrganization = id => {
    setOrganizationRequests(prevOrgs => prevOrgs.filter(org => org.id !== id));
  };
      ...
      <table className="w-full table-auto bg-white text-black-900 border-collapse border border-purple-900 mt-10 ">
        <thead>
          <tr className="bg-purple-800 text-white">
            ...
            <th className="border border-purple-900 px-4 py-2">Submissions</th>
            <th className="border border-purple-900 px-4 py-2">Actions</th>
          </tr>
        </thead>
        <tbody>
          {organizationRequests.map(org => (
            <tr key={org.id} className="border-t border-purple-900">
              <td className="border border-purple-900 px-4 py-2">{org.name}</td>
              <td className="border border-purple-900 px-4 py-2">
                <button onClick={() => handleViewSubmissions(org)} className="bg-purple-600 hover:bg-purple-700 text-white font-bold py-1 px-2 rounded">
                  View Submission
                </button>
                <a href="#" download="dummy-document.pdf" className="bg-purple-600 hover:bg-purple-700 text-white font-bold py-1 px-2 rounded ml-2">
                  Download
                </a>
              </td>
              <td className="border border-purple-900 px-4 py-2 flex justify-between">
                <div className="flex space-x-4">
                  <button onClick={() => handleAcceptOrganization(org.id)} className="text-green-600 border-none">
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path>
                    </svg>
                  </button>
                  <button onClick={() => handleRejectOrganization(org.id)} className="text-red-600 border-none">
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
